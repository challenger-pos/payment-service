...
public class MercadoPagoAdapter implements PaymentGatewayPort {
    ...
    @Override
    public PaymentResponse processPixPayment(BigDecimal amount, String documentNumber, String email, String name, String description) {
        log.info("Processing PIX payment through Mercado Pago Orders API: amount={}, email={}", 
            amount, email);
        ...
        try {
            ...
            MercadoPagoOrderRequest orderRequest = new MercadoPagoOrderRequest(
                externalReference,
                amount,
                payerEmail
            );
            ...
            ResponseEntity<MercadoPagoOrderResponse> responseEntity = restTemplate.exchange(
                ORDERS_API_URL,
                HttpMethod.POST,
                requestEntity,
            ...
            log.info("Mercado Pago order created: id={}, status={}", 
            ...
            if (status == PaymentStatus.REJECTED && orderResponse.getTransactions() != null && 
                orderResponse.getTransactions().getPayments() != null && 
                orderResponse.getTransactions().getPayments().length > 0) {
                errorMessage = orderResponse.getTransactions().getPayments()[0].getStatusDetail();
            }
            
            log.info("PIX payment processed: orderId={}, paymentId={}, status={}", 
