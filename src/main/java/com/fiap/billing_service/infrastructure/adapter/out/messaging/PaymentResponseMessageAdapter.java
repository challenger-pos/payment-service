...
public class PaymentResponseMessageAdapter implements PaymentResponseMessagePort {
    ...
    public void sendPaymentResponse(Payment payment) {
        try {
            ...
            String queueName = isSuccessStatus(payment.getStatus().name()) ? 
                successQueueName : failureQueueName;
            
            log.info("Sending payment response to queue: {} for paymentId={}, status={}", 
                queueName, payment.getId(), payment.getStatus());
            ...
            message.put("workOrderId", payment.getOrderId().toString());
            ...
            message.put("createdAt", payment.getCreatedAt() != null ? payment.getCreatedAt().toString() : null);
            message.put("processedAt", payment.getProcessedAt() != null ? payment.getProcessedAt().toString() : null);
            ...
            log.info("Payment response sent successfully to queue: {} with MessageId: {}", 
                queueName, result.messageId());

        } catch (Exception e) {
            log.error("Error sending payment response to SQS for paymentId={}", 
                payment.getId(), e);
